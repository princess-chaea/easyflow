...
프론트 협의용으로 `서식자료실_스마트 계획서 변환.html`에 `감지 → 지시 → 계획 → 비교 → 저장` 상호작용 데모가 있다. 이는 실제 문서를 읽거나 수정하지 않으며, 로컬 에이전트 연결 후 같은 화면 상태에 실제 응답을 대입한다.

## 2. 구현 판정

브라우저만으로 별도 데스크톱 한글 창을 제어할 수는 없다. 다음 구성요소가 필요하다.
...
`서식자료실_스마트 계획서 변환.html`의 협의용 데모는 다음 동작을 실제 프론트 상태로 구현한다.

- 계획 항목별 선택과 비교 단계 전달
- 변경별 근거, 위험 수준, 보존 조건 표시
- 변경별 수락·거절 및 미결정 상태 차단
- 선택한 연산만 포함한 `easyflow.document-patch/v1` JSON 다운로드
- 승인 결과와 미수행 검증을 포함한 `easyflow.document-apply-receipt/v1` JSON 다운로드

이 화면은 실제 문서 I/O를 성공으로 가장하지 않는다. 로컬 에이전트가 연결되지 않은 데모 영수증은
`demo: true`, `outcome: "demo_only"`, `documentChanged: false`이며 백업·패키지 무결성·한글 재열기·
시각 검증을 모두 `not_performed`로 반환한다.
...
확인 기준일은 2026-08-15이다. 저장소의 공개 여부나 비상업 개발이라는 사실만으로 코드 사용 권한이
...
|---|---|---|---|
| [edwardkim/rhwp](https://github.com/edwardkim/rhwp) | 루트 `LICENSE`의 MIT | 브라우저 HWP/HWPX 미리보기·보조 편집, WASM 파서 | 저작권·MIT 전문 포함, 사용 버전 고정, v1.0 전 레이아웃을 한컴 최종 판정 대신 사용 금지 |
...
| 브라우저 미리보기·보조 편집 | `rhwp` WASM PoC | 서버 렌더 | 한컴과 픽셀 동일하다고 표시하지 않음 |
